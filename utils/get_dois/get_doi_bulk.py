...
def get_args():
    ...
    parser.add_argument("path", help=r"Full path to text file contiaining urls: C:\..\..\urls.txt",
...
def pull_doi(url):
    ...
    if len(doi) == 0:
        ...
        try:
            doi = soup(text=re.compile(
                r'/^10.\d{4,9}/[-._;()/:A-Z0-9]+$/i'))[0].strip()

        except Exception as error:
            # Log errors
            with open("error_logs.txt", "a") as error_log:
                error_log.write(datetime.datetime.now().strftime(
                    r"%Y/%m/%d @ %H:%M:%S"))
                error_log.write("\n")
                error_log.write("URL: {}\n".format(url))
                error_log.write("{}\n\n".format(error))
